...
- `components/layout/AnimacionesSitio.tsx` — **director de animaciones GSAP de TODO el sitio** (2026-07-11; reemplaza la coreografía de portada revertida). Client montado al final de `(sitio)/layout.tsx`; las páginas server solo se marcan con `data-anim`: `hero` (timeline de entrada del hero de portada, incl. sello "Nota destacada" con back.out), `cabecera` (hijos en cascada al cargar), `carga` (elemento suelto en la cascada de carga), `grupo` (hijos en cascada al scrollear, ScrollTrigger `once`), `aparece` (bloque individual on-scroll), `cronologia` (timeline del hub de jugador, entra desde la izquierda). Movimiento de la casa: `power4.out`, 0.55s, 24px, sin rebotes ni clip-paths. El estado oculto lo setea GSAP por JS (sin JS / reduced-motion / import fallido = contenido visible); `clearProps: "transform"` al terminar para no pisar los hovers `.brut-cta-*`; usa `useSearchParams` con Suspense propio (fallback null, mismo patrón que TraspasosLink — NO sacarle el boundary). ⚠️ Al testear con Playwright: si la ventana está tapada, el rAF se estrangula y las animaciones "se congelan" — usar `bringToFront()` y scroll con `mouse.wheel` (Lenis revierte `window.scrollTo` instantáneo).
