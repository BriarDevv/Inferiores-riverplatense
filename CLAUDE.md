...
- **`components/layout/AnimacionesPortada.tsx`** = director único (client, render null, montado solo en la rama editorial de `page.tsx`): importa GSAP + ScrollTrigger + **SplitText** dinámicamente (cero GSAP en el bundle inicial; SplitText es gratis desde GSAP 3.13 y opcional — si falla el import, el título entra entero) y anima elementos marcados con `data-anim` — las cards/páginas siguen siendo server components. Contrato v2 ("más chiches", pedido 2026-07-11 tras quedar corta la v1 sobria): `hero` (timeline: imagen clip-path izq→der + zoom 1.15→1 que asienta, badge "Nota destacada" desde el borde, kicker, **título línea por línea enmascarado** con SplitText — espera `document.fonts.ready` y se revierte `onComplete` para devolver el DOM original —, bajada/byline/CTA en fila, y **estampado con golpe**: la sombra aparece de un frame + el marco acusa el impacto con un jolt de 4px), `grupo` (stagger 80ms), `cascada` (filas de NoticiasList/UltimasList desfilan desde la izquierda), `overline` (micro-slide x), `sello` (bandas: entran + sombra estampada + jolt de 3px), `contador` (números Anton de SobreAutorBand cuentan de 0 al real con `snap`). Todo `power3/power4.out`, `once: true`, trigger `top 85%`. Tras animar, los `clearProps: "transform"` devuelven los hovers CSS (el zoom del hero necesita ese clear).
...
### ✅ Próximo partido en la barra roja (2026-07-08)

La barra roja anuncia el próximo partido, cargado **a mano** desde el panel (no hay API confiable de fixtures de Reserva/juveniles — decisión conversada con el usuario):

- **Tabla `proximo_partido`** (migración `006`): singleton (`id boolean pk default true check (id)`), campos rival/division/fecha(timestamptz)/torneo. RLS: lectura pública, escritura staff (`es_editor()`).
- **`lib/partido.ts`** (solo server): `getProximoPartido()` (cliente anónimo, cacheado), `partidoVigente()` / `expiraPartido()` (**gracia de 3h post partido**, luego el anuncio muere solo), `formatearFechaPartido()` ("vie 10/07 · 12:00", **armado por partes** porque el formato combinado de es-AR cambia "/" por "-" según la versión de ICU de Node), `fechaLocalBuenosAires()` (ISO→datetime-local). Tests en `lib/partido.test.ts`.
- **Panel `/admin/partido`** (sidebar "Próximo partido"): `FormPartido` (rival/división/fecha/torneo opcional + vista previa + "Quitar de la barra" con ConfirmDialog). La página le pasa `key` con los datos → el form se remonta si el registro cambia (patrón React de reset; por eso `no-derived-useState` está eximido para ese archivo). `lib/admin/partido-actions.ts` = actions con sesión inline + `.select("id")` + `revalidatePath("/", "layout")` (la barra vive en el layout).
- **Fechas**: el datetime-local del panel se interpreta como hora de Buenos Aires (`-03:00` fijo, sin DST). El label de la barra se formatea SOLO en el server (cero riesgo de hydration mismatch).
- **Seed**: `PARTIDO_SEED` en `scripts/seed-data.ts` (la final vs Racing, vie 10/7 12:00, dato de nr-05); `seed.ts` lo upsertea (⚠️ pisa lo cargado por el panel — el seed es solo para armar la demo).
- Cargado en DB: **Racing · Reserva · vie 10/07 12:00 · "Final · Torneo Proyección"** — el viernes a las ~15:00 desaparece solo y la barra vuelve a "Último".

### ✅ Pasada react-doctor (2026-07-07) — score 100/100

`npx react-doctor` sobre sitio+admin: 101 hallazgos → 0 (score 27 → **100 "Great"**). Se corrigió todo lo real; lo intencional quedó eximido en **`doctor.config.json`** (JSON5 comentado, cada excepción con su porqué — mantenerlo al día si se agregan reglas). **2026-07-08, v0.7.2 trajo reglas nuevas**: se re-fixeó (Promise.all en /division, Set en GraficoBarras, lazy init de tags en EditorNota) y se eximieron con comentario: index-en-key de listas estáticas de split() (nota page + PreviewNota), falso positivo de `.includes` de STRING en lib/notas, y el estado sembrado de FormPartido (remonta por key) — sigue 100/100. Ojo: rutas con `(sitio)`/`[slug]` en los overrides van con comodines (`app/*/nota/*/page.tsx`) porque paréntesis/corchetes son glob chars. Fixes de la última milla: `RegistrarVisita` usa `navigator.sendBeacon` (mejor que fetch para telemetría), `equipo-actions` importa `server-only`, array vacío congelado a module scope en Estadísticas, autoFocus redundante fuera de PreviewNota (showModal ya enfoca).
...
- `components/cards/NotaCard.tsx` — 3 variantes según `variant`/`formato`: `short` (9:16), `youtube` (16:9), `articulo` (4:5). Frame 2px ink, hover revela offset shadow. ⚠️ **Patrón de altura (2026-07-09, pedido del usuario en dos pasos)**: ArticleCard y TeaserCard usan `h-full flex flex-col` + **imagen con `grow`** (aspect-ratio como base) + footer `shrink-0` con la meta pegada al título. Las filas quedan SIMÉTRICAS (todas las cards de la fila miden igual) y el sobrante lo absorbe la IMAGEN (object-cover recorta), nunca un hueco blanco entre título y firma. NO volver ni a `mt-auto` en la meta (deja vacío interno) ni a `self-start` (bordes inferiores desparejos). Las grillas de TeaserCards del sitio van SIN `items-start` para que la fila estire.
- `components/cards/TeaserCard.tsx` — card de los 3 teasers bajo el hero. Imagen 4:3 + footer. Mismo patrón de altura que ArticleCard (imagen `grow`, footer `shrink-0`).
...
**NIVEL 1 — barra roja**: fecha `es-AR` + Buenos Aires (izq) · a la derecha (oculto en `<md`, truncado con ellipsis): **"Próximo · vs {rival} · {división} · {vie 10/07 · 12:00}"** si hay partido cargado y vigente (link a la landing de la división), si no **"Último · {título de la última nota}"** (link a la nota). Los datos vienen del server: `(sitio)/layout.tsx` (async) arma `ultima` + `partido` (strings ya formateados vía `lib/partido.ts`) y se los pasa al Nav por props — sin CLS. El Nav re-chequea la vigencia al hidratar (`useSyncExternalStore`): si la página estática quedó servida y el partido ya pasó, cae solo a "Último". ⚠️ El 2026-07-08 el usuario pidió SACAR de esta barra el kicker "De la Novena a la Primera", Sobre, Contacto, las redes y "Nº 001" — no restaurarlos acá (Sobre/Contacto siguen en footer + panel mobile). Ver sección **"Próximo partido"**.
